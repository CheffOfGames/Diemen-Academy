...
class HomeScreen(Screen):
    def __init__(self, root: Tk, screens: dict, database, user="", usertype:int=-1):
        ...
        if self.usertype == "Student": # Student
            ...
            button_student = Button(root,text="Student",width=int(((self.width/50)-(self.width/500))*1.5), height=int((self.height/100)-(self.height/160)),command = lambda: self.changeScreen("AdminExam")).place(x= (self.width/3*1.5), y= (self.height/3*1.5),anchor = 'center')